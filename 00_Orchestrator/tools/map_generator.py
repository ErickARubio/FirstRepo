...
Uso:
    python 00_Orchestrator/tools/map_generator.py
    python 00_Orchestrator/tools/map_generator.py --map M01

Requisitos: matplotlib numpy  (en requirements.txt)
Salida: 03_Assets/maps/M0N_*.png  (1920x1080 px)
"""
...
OUTPUT_DIR = ROOT / "03_Assets" / "maps"
CACHE_DIR  = ROOT / "00_Orchestrator" / "cache"
...
def create_M01(features_mx, _) -> Path:
    out = OUTPUT_DIR / "M01_remesas_estados.png"
    if out.exists():
        print(f"  [SKIP] M01 ya existe ({out.stat().st_size // 1024} KB)")
        return out
    print("  M01 Remesas por estado 2024...", end=" ", flush=True)

    norm = mcolors.Normalize(vmin=98, vmax=4241)
    fig, ax = setup_ax((-118, -86, 14, 33))
    render_choropleth(ax, features_mx, REMESAS_2024, GOLD_CMAP, norm, is_mx=True)
    add_colorbar(fig, ax, GOLD_CMAP, norm, "Millones USD")
    add_title_block(ax,
        "Remesas recibidas por estado — 2024",
        "Total nacional: $67,637 millones USD  |  Top 3: Michoacan, Guanajuato, Jalisco",
        "Banco de Mexico (Banxico), Balanza de Pagos 2024")
...
def create_M02(features_mx, _) -> Path:
    out = OUTPUT_DIR / "M02_pib_crecimiento.png"
    if out.exists():
        print(f"  [SKIP] M02 ya existe ({out.stat().st_size // 1024} KB)")
        return out
    print("  M02 Crecimiento PIB per capita 2010-2023...", end=" ", flush=True)

    norm = TwoSlopeNorm(vmin=-20, vcenter=0, vmax=50)
    fig, ax = setup_ax((-118, -86, 14, 33))
    render_choropleth(ax, features_mx, PIB_CRECIMIENTO_2010_2023, DIVG_CMAP, norm, is_mx=True)
    add_colorbar(fig, ax, DIVG_CMAP, norm, "% crecimiento acumulado")
    add_title_block(ax,
        "Crecimiento PIB per capita estatal — 2010 a 2023",
        "Queretaro +48.7% vs Campeche -18.3%  |  Estados remeseros: crecimiento lento",
...
def run():
    ...
    for name, fn in target.items():
        try:
            out = fn(features_mx, features_us)
